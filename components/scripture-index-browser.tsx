...
export default function ScriptureIndexBrowser({ books }: { books: IndexBook[] }) {
  ...
  // OT first, NT second — matches sidebar order
  ...
  return (
    <div
      ...
      style={{ display: 'grid', gridTemplateColumns: '1fr', paddingTop: '2.5rem', gap: '3.5rem' }}
    >
      <div
        className="lg:grid"
        style={{ display: 'contents' }}
      >
        {/* Outer wrapper that makes sticky work inside a grid */}
        <div className="hidden lg:contents">
          {/* Sticky sidebar — rendered before content in DOM for correct grid placement */}
        </div>
      </div>

      <div className="lg:grid lg:gap-14 lg:items-start" style={{ gridTemplateColumns: '200px 1fr' }}>

        {/* Sidebar */}
        <aside
          className="hidden lg:block"
          style={{ position: 'sticky', top: 'calc(60px + 2rem)', maxHeight: 'calc(100vh - 100px)', overflowY: 'auto' }}
        >
          <div
          ...
          <ul className="flex flex-col border-l" style={{ borderColor: '#E2DACE' }}>
            ...
            {otBooks.length > 0 && (
              <>
                <li>
                  <a
                    href="#old-testament"
                    className="flex items-center px-4 py-1.5 text-[0.72rem] font-medium tracking-[0.06em] uppercase transition-colors"
                    style={{ color: '#9A9189' }}
                  >
                    Old Testament
                  </a>
            ...
            {ntBooks.length > 0 && (
              <>
                <li className={otBooks.length > 0 ? 'mt-2' : ''}>
                  <a
                    href="#new-testament"
                    className="flex items-center px-4 py-1.5 text-[0.72rem] font-medium tracking-[0.06em] uppercase transition-colors"
                    style={{ color: '#9A9189' }}
          ...
          <Link
            href="/sermons"
            className="inline-flex items-center gap-1.5 text-[0.75rem] transition-colors pl-0.5 hover:text-[#7A5C1E]"
            style={{ color: '#9A9189' }}
          >
        ...
        <div>
          ...
          <div className="relative mb-10">
            ...
            {query && (
              <button
                onClick={() => setQuery('')}
                className="absolute right-4 top-1/2 -translate-y-1/2 transition-colors hover:text-[#1A1714]"
                style={{ color: '#9A9189' }}
                aria-label="Clear"
              >
  ...
  return (
    <li>
      ...
      >
        {book}
        <span
          className="text-[0.68rem] rounded-full px-2 py-0.5 ml-2 shrink-0"
...
function TestamentHeading({ children, id }: { children: React.ReactNode; id: string }) {
  return (
    <div
      id={id}
      className="flex items-center gap-2.5 text-[0.65rem] font-medium tracking-[0.14em] uppercase mb-7 scroll-mt-24"
      style={{ color: '#9A9189' }}
    >
      {children}
      ...
      <div className="flex flex-col">
        {data.sermons.map((s) => (
          <Link
            key={s.slug}
            href={`/sermons/${s.slug}`}
            className="group py-3.5 border-b last:border-0 rounded hover:bg-[#F2EFE7] transition-colors -mx-3 px-3"
            style={{ borderColor: '#E2DACE' }}
          >
            <div
              className="grid gap-4 items-baseline"
              style={{ gridTemplateColumns: s.passage ? '110px 1fr auto' : '1fr auto' }}
            >
              {s.passage && (
                <span
              ...
              >
                {s.formattedDate}
                <span className="h-[2px] w-[2px] rounded-full inline-block" style={{ background: '#C8BFA8' }} />
